...
if(isset($_POST["username"]) && isset($_POST["password"]))
{
	...
	$tm = htmlspecialchars($_POST["trytime"]);
		...
		{
			$work = new staffLogin($dbcon, $tb, $lock, $level);	
				$work_login = $work->loginRequest($un, $pw, $username, $password, $name, $token);
					if($work->err == false)
					{
						// print_r( $work->output);
						...
						// $_SESSION["name"] = $name;
							$post_data = array("1" => "yes", "2" => "$userkey", "3" => "$un", "4"=>"$level");
					...
					else
					{
						$err_2 = errorReporting($work->err_msg);
							$post_data = array("1" => "no", "2" => "$err_2", "3" => ".", "4"=>"null");
							$post_data = json_encode($post_data);
							print($post_data);
...
//confirming staff' with temporary key across pages
if(isset($_POST["chkkey"]) && isset($_POST["ipaddress"]))
{
	$idkey = htmlspecialchars($_POST["chkkey"]);
	$ip = htmlspecialchars($_POST["ipaddress"]);
	// print("hello");
		if($dbcon === false)
		{
		...
		{
			$work = new staffLogin($dbcon, $tb, $lock, $level);	
				$work_login = $work->verifierRequestMini($idkey, $token);
				// echo "result=" . $work->output;
					if($work->err == false)
					{
						$suc = $work->output;
...
function levelCheck($data)
{
	if($data == "0")
	{
		$level_redir = "admin/index.php?";
	}
	else if($data == "1")
	{
		$level_redir = "staff/index.php?";
	}
	else
	{
		$level_redir = "../../";
